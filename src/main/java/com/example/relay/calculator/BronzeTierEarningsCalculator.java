...
@Component
public class BronzeTierEarningsCalculator implements EarningsCalculator{

    Logger LOGGER = Logger.getLogger(BronzeTierEarningsCalculator.class.getName());
    final static String CARD_TIER = "bronze_tier";
    final static double MINIMUM_EARNING = 14.5;
    final static Map<String,Double> lineItemsRates =
            Map.of("Per successful attempt",0.459,
                    "Per unsuccessful attempt" , 0.229,
                    "Long route bonus" , 10.00,
                    "Loyalty bonus (routes)" , 20.00);

    //Create a map to store the Ids of different routes - unnecessary but used to take advantage of o(1) time complexity while searching the map later
    Map<String,Boolean> routeIdsMap = new HashMap<>();
    ...
    private double getFinalEarnings (double subtotal){
        //More than 30 successful drops
        if(lineItemsMap.get("Per successful attempt")> 30){
            subtotal += lineItemsRates.get("Long route bonus");
            lineItemsMap.put("Long route bonus",1);
        }

        //Participated in 10 different routes
        if(routeIdsMap.size()>10){
            subtotal += lineItemsRates.get("Loyalty bonus (routes)");
            lineItemsMap.put("Loyalty bonus (routes)",1);
        }
    ...
    private void populateMaps(List<Activity> activities){
        for(Activity activity: activities){
            ...
            //Add all the unique routeIds to the map
            if(!routeIdsMap.containsKey(activity.getRouteId())){
                routeIdsMap.put(activity.getRouteId(),true);
            }
